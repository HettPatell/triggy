-- Swiggy Clone Database Schema (MySQL Compatible)
-- Database Name: swiggy_clone

CREATE DATABASE IF NOT EXISTS `swiggy_clone` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `swiggy_clone`;
...
CREATE TABLE IF NOT EXISTS `restaurants` (
  ...
  `price_for_two` INT DEFAULT 350,
  `discount_text` VARCHAR(100) DEFAULT '60% OFF UPTO ₹120',
...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  `delivery_instructions` VARCHAR(255) DEFAULT '',
...
CREATE TABLE IF NOT EXISTS `favorites` (
  ...
  `item_type` ENUM('restaurant', 'menu_item') DEFAULT 'restaurant',
  `item_id` INT NOT NULL,
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY `user_favorite` (`user_id`, `item_type`, `item_id`),
...
INSERT INTO `restaurants` (`id`, `name`, `slug`, `image_url`, `cuisine`, `rating`, `rating_count`, `delivery_time_mins`, `price_for_two`, `discount_text`, `is_veg_only`, `address`) VALUES
...
INSERT INTO `menu_items` (`restaurant_id`, `category_id`, `name`, `description`, `price`, `image_url`, `is_veg`, `is_bestseller`) VALUES
-- Meghana Foods (ID: 1)
...
-- Seed Demo User (password: swiggy123)
-- Hash generated using password_hash('swiggy123', PASSWORD_BCRYPT)
INSERT INTO `users` (`id`, `name`, `email`, `phone`, `password_hash`, `address`) VALUES
(1, 'Rahul Sharma', 'rahul@example.com', '9876543210', '$2y$10$w850GZ1bXfR7tLdFzJm61eTsqM.M2mYjY7pQ.uCgE0rW3U4t4qXiq', 'Flat 402, Sunshine Heights, Koramangala 4th Block, Bangalore - 560034');
